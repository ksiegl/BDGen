...
void BDA021Z11(double *TOF,int *w_sign,double *pe,double *pv,double *p1,double *pg,double *pn, 
	       double *pe_exp,double *pv_exp,double *p1_exp,double *pg_exp,double *pn_exp,
	       double *loc,double *loc_n,
	       double *beta_rad, double *MCP_hit,double uran(long *),long *sptr,int choice)
{
...
double betadetector_hit(double *pe, double *loc, double *pe_exp, double uran(long *), long *sptr)
{
  // right now I am assuming there is only one double-sided silicon strip detector located in the +x direction
	...
	double EnergySigma;
	double xprime,yprime,zprime,pex,pey,pez;
	
	double angle = 45;

	 //return(1.0);  // this is in here so that all beta are considered uncommented takes out focusing
	
	// This rotation is in the x-y plane
	/*
	xprime = ((loc[0]-CENTER_X)*cos(angle*3.14159/180)) - ((loc[1]-CENTER_Y)*sin(angle*3.14159/180));
	yprime = ((loc[0]-CENTER_X)*sin(angle*3.14159/180)) + ((loc[1]-CENTER_Y)*cos(angle*3.14159/180));
	zprime = (loc[2]-CENTER_Z);
	
	pex = (pe[1]*cos(angle*3.14159/180)) - (pe[2]*sin(angle*3.14159/180));
	pey = (pe[1]*sin(angle*3.14159/180)) + (pe[2]*cos(angle*3.14159/180));
	pez = pe[3];
	*/
	// This rotation is in the y-z plane
	/*
	xprime = (loc[0]-CENTER_X);
	yprime = ((loc[1]-CENTER_Y)*cos(angle*3.14159/180)) - ((loc[2]-CENTER_Z)*sin(angle*3.14159/180));
	zprime = ((loc[1]-CENTER_Y)*sin(angle*3.14159/180)) + ((loc[2]-CENTER_Z)*cos(angle*3.14159/180));
	
	pex = pe[1];
	pey = (pe[2]*cos(angle*3.14159/180)) - (pe[3]*sin(angle*3.14159/180));
	pez = (pe[2]*sin(angle*3.14159/180)) + (pe[3]*cos(angle*3.14159/180));
	*/
	// This rotation is in the x-z plane
	
	xprime = ((loc[0]-CENTER_X)*cos(angle*3.14159/180)) - ((loc[2]-CENTER_Z)*sin(angle*3.14159/180));
	yprime = (loc[1]-CENTER_Y);
	zprime = ((loc[0]-CENTER_X)*sin(angle*3.14159/180)) + ((loc[2]-CENTER_Z)*cos(angle*3.14159/180));
	
	pex = (pe[1]*cos(angle*3.14159/180)) - (pe[3]*sin(angle*3.14159/180));
	pey = pe[2];
	pez = (pe[1]*sin(angle*3.14159/180)) + (pe[3]*cos(angle*3.14159/180));
	
	
	//printf("%lf %lf %lf\n",pex,pey,pez);
	/*if(pey > 0) {
	
		x = xprime+(pex/pey)*(yprime-BD_X);
		y = yprime-BD_X;
		z = zprime+(pez/pey)*(yprime-BD_X);
	*/
	if(pex > 0) {
		
	    x = xprime-BD_X;
	    y = yprime+(pey/pex)*(xprime-BD_X);;
	    z = zprime+(pez/pex)*(xprime-BD_X);
